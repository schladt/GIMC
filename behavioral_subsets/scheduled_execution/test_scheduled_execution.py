...
class ScheduledExecutionTest:
    ...
    def get_results(self):
        ...
        all_passed = all(passed for _, passed, _ in metrics)
        num_failures = sum(1 for _, passed, _ in metrics if not passed)
        ...
        return {
            ...
            "num_errors": 0,
            "details": {
                "install_success": self.results['install_success'],
                "exit_code": self.results['exit_code'],
                "file_created": self.results['file_created'],
                "recurring_execution": self.results['recurring_execution'],
                "update_count": self.results['update_count'],
...
def run(exe_path, test_file=None):
    """
    Run the scheduled execution unit test.
    
    This function can be called directly by the build agent to avoid subprocess overhead.
    
    Args:
        exe_path: Path to the BSI executable to test
        test_file: Optional path to test file (default: C:\\Users\\Public\\gimc_test.log)
    
    Returns:
        dict: Test results in format {"num_tests": int, "num_failures": int, "num_errors": int, "details": dict}
    """
    if test_file is None:
        test_file = 'C:\\Users\\Public\\gimc_test.log'
    
    test = ScheduledExecutionTest(exe_path, test_file)
    test.run_full_test()
...
This test validates three key behaviors:
  ...
  3. Payload executes repeatedly (3+ times in 10 seconds)
    ...
    parser.add_argument(
        ...
        default='C:\\Users\\Public\\gimc_test.log'
    )
    
    args = parser.parse_args()
    
    # Run the test and print results
    results = run(args.executable, args.test_file)
    
    # Print JSON results for parsing
    print("\n" + json.dumps(results))
    
    # Exit with appropriate code
    sys.exit(0 if results['num_failures'] == 0 else 1)
